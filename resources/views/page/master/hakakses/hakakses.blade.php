...
@section('content')
    ...
    <div id="modal_tambah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="form_tambah" method="post" action="{!! url('master/hakakses/store') !!}">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="tambah_nama" name="nama" placeholder="Nama Hak Akses" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
    ...
    <div id="modal_ubah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="form_ubah" method="post" action="{!! url('master/hakakses/update') !!}" >
                    <div class="modal-header">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="ubah_nama" name="nama" placeholder="Nama Hak Akses" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            tabel = $('#table').DataTable({
                processing: false,
                serverSide: true,
                ajax: '{!! url('master/hakakses/get') !!}',
                order: [],
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
                    { data: 'hak_akses_nama', name: 'hak_akses_nama' },
                    { data: 'hak_akses_keterangan', name: 'hak_akses_keterangan' },
                    { data: 'hak_akses_status', "render": function ( data, type, row ) 
                        {
                            if (data == 1) {
                                return `<button type="button" class="btn btn-success waves-effect waves-light btn-sm" onclick="ubahStatus(${row.hak_akses_id}, ${data})">Aktif</button>`;
                            } else {
                                return `<button type="button" class="btn btn-danger waves-effect waves-light btn-sm" onclick="ubahStatus(${row.hak_akses_id}, ${data})">Tidak Aktif</button>`;
                            }
                        },
                        searchable: false
                    },
        ...
        function ubah(row) {
            $('#ubah_id').val(row.hak_akses_id);
            $('#ubah_nama').val(row.hak_akses_nama);
            $('#ubah_keterangan').val(row.hak_akses_keterangan);
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return new Promise(function (resolve) {
                        $.ajax({
                            url: `{!! url('master/hakakses/status') !!}/${id}`,
                            type: 'put',
                            data: {'status': status},
                            dataType: 'json',
