    ...
    <div id="modal_tambah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="form_tambah" method="post" action="{!! url('master/komoditas/store') !!}">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="tambah_nama" name="nama" placeholder="Nama Komoditas" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            tabel = $('#table').DataTable({
                processing: false,
                serverSide: true,
                ajax: '{!! url('master/komoditas/get') !!}',
                order: [],
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
                    { data: 'komoditas_nama', name: 'komoditas_nama' },
                    { data: 'komoditas_keterangan', name: 'komoditas_keterangan' },
                    { data: 'komoditas_status', "render": function ( data, type, row ) 
                        {
                            if (data == 1) {
                                return `<button type="button" class="btn btn-success waves-effect waves-light btn-sm" onclick="ubahStatus(${row.komoditas_id}, ${data})">Aktif</button>`;
                            } else {
                                return `<button type="button" class="btn btn-danger waves-effect waves-light btn-sm" onclick="ubahStatus(${row.komoditas_id}, ${data})">Tidak Aktif</button>`;
                            }
                        },
                        searchable: false
                    },
        ...
        function ubah(row) {
            $('#ubah_id').val(row.komoditas_id);
            $('#ubah_nama').val(row.komoditas_nama);
            $('#ubah_keterangan').val(row.komoditas_keterangan);
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return new Promise(function (resolve) {
                        $.ajax({
                            url: `{!! url('master/komoditas/status') !!}/${id}`,
                            type: 'put',
                            data: {'status': status},
                            dataType: 'json',
