...
@section('content')
    ...
    <div id="modal_tambah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="form_tambah" method="post" action="{!! url('master/ruang/tipe/store') !!}">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="tambah_nama" name="nama" placeholder="Nama Tipe Ruang" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="tambah_tipe" class="control-label">Komoditas</label>
                                    <select class="form-control select2" id="tambah_komoditas" name="komoditas" required>
                                        <option value="" selected disabled> -- Pilih Komoditas -- </option>
                                        @foreach ($komoditas as $key => $value)
                                            <option value="{{ $value->komoditas_id}}" > {{ $value->komoditas_nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <div id="modal_ubah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="form_ubah" method="post" action="{!! url('master/ruang/tipe/update') !!}" >
                    <div class="modal-header">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="ubah_nama" name="nama" placeholder="Nama Tipe Ruang" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="ubah_tipe" class="control-label">Komoditas</label>
                                    <select class="form-control select2" id="ubah_komoditas" name="komoditas" required>
                                        <option value="" selected disabled> -- Pilih Komoditas -- </option>
                                        @foreach ($komoditas as $key => $value)
                                            <option value="{{ $value->komoditas_id}}" > {{ $value->komoditas_nama }}</option>
                                        @endforeach
...
@section('customJS')
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#tambah_komoditas').select2({dropdownParent: $("#modal_tambah")});

            tabel = $('#table').DataTable({
                processing: false,
                serverSide: true,
                ajax: '{!! url('master/ruang/tipe/get') !!}',
                order: [],
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
                    { data: 'ruang_tipe_nama', name: 'ruang_tipe_nama' },
                    { data: 'komoditas_nama', name: 'komoditas.komoditas_nama' },
                    { data: 'ruang_tipe_status', "render": function ( data, type, row ) 
                        {
                            if (data == 1) {
                                return `<button type="button" class="btn btn-success waves-effect waves-light btn-sm" onclick="ubahStatus(${row.ruang_tipe_id}, ${data})">Aktif</button>`;
                            } else {
                                return `<button type="button" class="btn btn-danger waves-effect waves-light btn-sm" onclick="ubahStatus(${row.ruang_tipe_id}, ${data})">Tidak Aktif</button>`;
                            }
                        },
                        searchable: false
                    },
        ...
        function ubah(row) {
            $('#ubah_id').val(row.ruang_tipe_id);
            $('#ubah_nama').val(row.ruang_tipe_nama);
            $('#ubah_komoditas').val(row.komoditas_id);
            $('#ubah_komoditas').select2({dropdownParent: $("#modal_ubah")}).trigger('change');
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    return new Promise(function (resolve) {
                        $.ajax({
                            url: `{!! url('master/ruang/tipe/status') !!}/${id}`,
                            type: 'put',
                            data: {'status': status},
                            dataType: 'json',
