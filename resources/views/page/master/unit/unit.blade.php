...
@section('content')
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="tambah_tipe" name="tipe" required>
                                        <option value="" selected disabled> -- Pilih Tipe Unit -- </option>
                                        @foreach ($tipe as $key => $value)
                                            <option value="{{ $value->unit_tipe_id}}" > {{ $value->unit_tipe_nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <div id="modal_ubah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="form_ubah" method="post" action="{!! url('master/unit/update') !!}" >
                    <div class="modal-header">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="ubah_nama" name="nama" placeholder="Nama Unit" required>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="ubah_tipe" class="control-label">Tipe Unit</label>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            tabel = $('#table').DataTable({
                processing: false,
                serverSide: true,
                ajax: '{!! url('master/unit/get') !!}',
                order: [],
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
                    { data: 'unit_nama', name: 'unit_nama' },
                    { data: 'unit_tipe_nama', name: 'unit_tipe.unit_tipe_nama' },
                    { data: 'unit_keterangan', name: 'unit_keterangan' },
                    { data: 'unit_status', "render": function ( data, type, row ) 
                        {
                            if (data == 1) {
                                return `<button type="button" class="btn btn-success waves-effect waves-light btn-sm" onclick="ubahStatus(${row.unit_id}, ${data})">Aktif</button>`;
                            } else {
                                return `<button type="button" class="btn btn-danger waves-effect waves-light btn-sm" onclick="ubahStatus(${row.unit_id}, ${data})">Tidak Aktif</button>`;
                            }
                        },
                        searchable: false
                    },
